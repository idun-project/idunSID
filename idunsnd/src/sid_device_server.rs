...
use player::Player;
use player::code_renderer::NMIPORT;
...
impl SidDeviceServerThread {
    pub fn new(config: Arc<Mutex<Config>>) -> SidDeviceServerThread {
        ...
        let number_of_devices = if Path::new(NMIPORT).exists() { 3 } else { 2 };
    ...
    fn process_command(&mut self, stream: &mut TcpStream, data: &[u8]) -> io::Result<()> {
        ...
        match command {
            ...
            Command::GetConfigCount => {
                stream.write_all(&[CommandResponse::Count as u8, self.number_of_devices])?;
            }
            Command::GetConfigInfo => {
                ...
                } else if sid_number == 1 {
                    response.append(&mut b"reSID Device (8580)\0".to_vec());
                } else {
                    response.append(&mut b"Real SID (6581)\0".to_vec());
                }
            ...
            Command::Mute => {
                if data_length == 2 {
                    if data[1] == 0 { self.player.set_mute(data[0]) };
                    stream.write_all(&[CommandResponse::Ok as u8])?;
                } else {
                    println!("ERROR: Mute missing data\r");
                    stream.write_all(&[CommandResponse::Error as u8])?;
                }
            }
            _ => {
